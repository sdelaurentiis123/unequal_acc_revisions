...
for row, (eb, qb) in enumerate(sim_list):
    ...
    twinax.set_ylim(0, 5)
    twinax.set_ylabel(r'$r_1 \, [a]$', fontsize=13)
    ...
    # Mark the dominant peak shared by lambda and r_1 (use lambda's peak)
    mask = (P_lam >= PERIOD_MIN) & (P_lam <= PERIOD_MAX) & (A_lam > 0.05)
    if mask.any():
        i = np.argmax(A_lam * mask)
        ax.axvline(P_lam[i], color='gray', alpha=0.4, lw=0.8, linestyle='--')
        ax.text(P_lam[i] * 1.05, A_lam[i],
                rf'$\tau={P_lam[i]:.0f}\,\tau_b$',
                color='black', fontsize=9, va='bottom')

    ax.set_xscale('log')
    ax.set_xlim(PERIOD_MIN, PERIOD_MAX)
    ax.set_ylim(0, max(A_lam[1:].max(), A_r1[1:].max()) * 1.15)
