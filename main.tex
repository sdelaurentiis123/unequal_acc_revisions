...
\begin{figure}
    ...
    \begin{tikzpicture}[scale=2.0]
      ...
      \node[black, anchor=south] at (0, 1.10) {\footnotesize CBD inner edge};

      % --- center of mass: X marker (less dramatic than a filled dot) ---
      \draw[black, thick] (-0.04, -0.04) -- (0.04, 0.04);
      \draw[black, thick] (-0.04, 0.04) -- (0.04, -0.04);
      \node[black, anchor=north west, font=\scriptsize] at (0.02, -0.05) {COM};

      % --- binary at apocenter, q_b = 0.1, e_b = 0.2 ---
      % primary at d_1 = -0.109a, secondary at d_2 = +1.091a
      \fill[blue!70!black] (-0.109, 0) circle (0.06);
      \node[blue!70!black, anchor=east, font=\scriptsize] at (-0.18, 0.0) {$M_1$};
      \fill[red!70!black]  (+1.091, 0) circle (0.030);
      \node[red!70!black, anchor=south, font=\scriptsize] at (+1.091, 0.05) {$M_2$};

      % --- r_1 vector: primary to nearest cavity wall on its (far) side ---
      \draw[<->, blue!70!black, thick] (-0.109, 0.18) -- (-1.4, 0.18);
      \node[blue!70!black, anchor=south, font=\scriptsize] at (-0.75, 0.20) {$r_1$};

      % --- r_2 vector: secondary to nearest cavity wall on its (near) side ---
      \draw[<->, red!70!black, thick] (+1.091, -0.20) -- (+1.4, -0.20);
      \node[red!70!black, anchor=north, font=\scriptsize] at (+1.25, -0.22) {$r_2$};

      % --- binary axis line ---
      \draw[gray, dashed, thin] (-0.109, 0) -- (+1.091, 0);

      % --- precession indicator ---
      \draw[->, gray!60!black, thick] (-0.7, 0.85) arc[start angle=130, end angle=90, x radius=1.4, y radius=1.0];
      \node[gray!60!black, anchor=west, font=\scriptsize] at (-1.1, 0.95) {precession};
    \end{tikzpicture}
    \caption{Schematic of the cavity geometry at apocenter for the locked-CBD case, illustrated for an unequal-mass binary with $q_b \ll 1$. The CBD inner edge (black ellipse) is closer to the secondary BH ($M_2$, red) on one side; the primary ($M_1$, blue) is farther from its nearby cavity wall. Closest distances from each BH to the wall are $r_1$ and $r_2$, with $r_1 > r_2$ in the locked configuration shown. As the disk precesses (curved gray arrow), the ellipse's major axis rotates, and the orientations of $r_1$ and $r_2$ change accordingly.}
