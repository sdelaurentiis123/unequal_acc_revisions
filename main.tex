...
\autoref{fig:mdot_edd} and its discussion assumed a binary accretion rate of $1 \dot{M}_{\rm Edd}$. However, this assumption only controls the baseline of the binary accretion rate. While a different assumed binary accretion rate would not change the profiles nor the relative accretion rates of the components, it would affect the absolute amplitude --- changing the jet-launching behavior. The qualitative tapestry of regimes therefore shifts continuously with $\dot{M}_b$: at sub-Eddington rates ($\dot{M}_b \ll \dot{M}_{\rm Edd}$) no jets launch anywhere in $(e_b, q_b)$ space, while at strongly super-Eddington rates ($\dot{M}_b \gg \dot{M}_{\rm Edd}$) dual jets dominate the parameter space. The fiducial diagram in \autoref{fig:mdot_edd} represents one slice of this continuous family.
