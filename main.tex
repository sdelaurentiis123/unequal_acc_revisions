...
\begin{figure}
    ...
    \caption{Six representative $(e_b, q_b)$ simulations comparing the individual accretion rates $\dot{M}_1$ (purple, primary) and $\dot{M}_2$ (green, secondary) on the left log-axis to the BH-to-cavity-edge distances $r_1$ (blue, primary) and $r_2$ (red, secondary), in units of the binary semi-major axis $a$, on the right axis. Time is in binary orbital periods $\tau_b$; panels are plotted for $5000 \le t/\tau_b \le 10000$ to focus on the post-transient regime. Reading row-by-row from the top, the panels show $(e_b, q_b) = (0.5, 0.8)$, $(0.8, 1.0)$, $(0.2, 0.1)$, $(0.8, 0.3)$, $(0.4, 0.7)$, $(0.5, 0.7)$. The naive expectation that the BH closer to the cavity wall (smaller $r$) should accrete more rapidly is satisfied only for $(0.2, 0.1)$ (third panel); the other five panels show that proximity to the cavity wall does not directly determine which BH accretes preferentially.}
