...
In addition to preferential accretion, we also report the average rate of change of the mass ratio, $\langle \dot{q} \rangle$. Namely, we calculate $\dot{q}(t)$ as described in \autoref{eqn:q_dot} in \autoref{sec:methods}, make a time-cut at $3000\,\tau_b$ to ensure that early instabilities do not affect our results, and report the mean on the truncated time-series in units of $\dot{M}_b/M_b$ in \autoref{fig:qdot_heatmap}.
