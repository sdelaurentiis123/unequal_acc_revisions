...
\autoref{fig:mdot_edd} and its discussion assumed a binary accretion rate of $1 \dot{M}_{\rm Edd}$. However, this assumption only controls the baseline of the binary accretion rate. While a different assumed binary accretion rate would not change the profiles nor the relative accretion rates of the components, it would affect the absolute amplitude --- changing the jet-launching behavior. To make this dependence explicit, \autoref{fig:mini_jet_regimes} shows the jet-regime classification across $(e_b, q_b)$ for three representative binary accretion rates spanning the dynamic range over which all three regimes coexist. As $\dot{M}_b$ rises from $3\,\dot{M}_{\rm Edd}$ to $10\,\dot{M}_{\rm Edd}$, single jets recede toward low $q_b$ and low $e_b$, dual jets fill in across most of parameter space, and flickering jets concentrate in the upper-right corner of high $q_b$ and high $e_b$ where the two BHs' accretion rates straddle the threshold and cross it alternately. At rates well below this range no jets launch anywhere in $(e_b, q_b)$ space; at rates well above, dual jets dominate the entire parameter space. The fiducial diagram in \autoref{fig:mdot_edd} represents one slice of this continuous family.

\begin{figure*}
    \centering
    \includegraphics[width=1\linewidth]{mini_fig8_jet_regimes.pdf}
    \caption{Jet-regime classification as a function of $(e_b, q_b)$ for three values of the binary accretion rate $\dot{M}_b \in \{3, 5, 10\}\,\dot{M}_{\rm Edd}$. Each cell is colored by the regime (white: no jet; blue: single; purple: dual; green: flickering) using the same threshold and duration criteria as \autoref{fig:mdot_edd} ($\dot{M}_i > 1.1\,\dot{M}_{{\rm Edd},i}$ sustained for $\geq 50\,\tau_b$). The three values bracket the range over which all three regimes coexist: at $\dot{M}_b = 3$ singles dominate with dual at the low-$e_b$, low-$q_b$ edge and flickering along $e_b \approx 0.6$; at $\dot{M}_b = 5$ duals fill most of the grid and flickering migrates upward in $q_b$; at $\dot{M}_b = 10$ duals dominate and flickering occupies the high-$q_b$, high-$e_b$ corner.}
