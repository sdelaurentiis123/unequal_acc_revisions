...
What first strikes us in \autoref{fig:lambda_full} is the broad division of $\lambda(t)$ into being time-stable or time-varying, naturally framed in periodogram terms following \citet{dorazio_2013}. Broadly, many low $e_b$, low $q_b$ binaries are time-stable. Though some experience a sharp change in behavior in the first $2000$ orbits associated with an expected initial instability \citep{moriwaki_04, DeLaurentiis25}, they soon reach a near-constant value. This time-stable behavior is well exemplified by the $(e_b, q_b) = (0.3, 0.3)$ binary\footnote{Though some binaries (e.g.\ $(e_b, q_b) = (0.0, 0.3)$) experience noticeable modulation around this constant value, the dichotomy between time-varying and time-stable $\lambda(t)$ values remains clear.}. Others show time-variable behavior with oscillations in $\lambda(t)$ spanning two orders of magnitude, such as $(e_b, q_b) = (0.6, 1.0)$.
...
\autoref{fig:mdot_edd} and its discussion assumed a binary accretion rate of $1 \dot{M}_{\rm Edd}$. However, this assumption only controls the baseline of the binary accretion rate. While a different assumed binary accretion rate would not change the profiles nor the relative accretion rates of the components, it would affect the absolute amplitude --- changing the jet-launching behavior. To make this dependence explicit, \autoref{fig:mini_jet_regimes} reproduces \autoref{fig:mdot_edd} at two bracketing values of the binary accretion rate: a sub-Eddington case ($\dot{M}_b = 0.5\,\dot{M}_{\rm Edd}$) and a super-Eddington case ($\dot{M}_b = 5\,\dot{M}_{\rm Edd}$). At sub-Eddington rates, single jets dominate the parameter space because only the secondary's smaller Eddington threshold is crossed; a small handful of $q_b \approx 1$ cells fail to launch any jet, and the high-$q_b$, high-$e_b$ corner shows flickering where strongly anti-correlated oscillations push the BHs across threshold alternately. At super-Eddington rates, dual jets fill almost the entire parameter space, with single jets surviving only at low $q_b$, low $e_b$ where the primary remains too underfed to cross threshold. The fiducial diagram in \autoref{fig:mdot_edd} represents one slice of this continuous family.

\begin{figure*}
    \centering
    \includegraphics[width=1\linewidth]{mini_fig8_jet_regimes.pdf}
    \caption{Reproduction of \autoref{fig:mdot_edd} at two bracketing values of the binary accretion rate: $\dot{M}_b = 0.5\,\dot{M}_{\rm Edd}$ (left) and $\dot{M}_b = 5\,\dot{M}_{\rm Edd}$ (right). Each cell shows the per-Eddington-normalized accretion rates of the primary (black) and secondary (red) on a log y-axis spanning $[10^{-1}, 10^1]$ versus time in $\tau_b$ within the post-transient window $7500 \le t/\tau_b \le 9500$; the dashed gray horizontal marks the $\dot{M}_i = \dot{M}_{{\rm Edd},i}$ threshold. Backgrounds are colored by jet-regime classification using the same criteria as \autoref{fig:mdot_edd}: white (no jet), blue (single), purple (dual), green (flickering). The figure makes explicit how the jet-regime tapestry shifts continuously with $\dot{M}_b$ between the two extremes.}
...
\begin{enumerate}
    ...
    \item We do not find evidence that a BH must be closer to the CBD cavity than its counterpart to accrete preferentially at a higher rate.
