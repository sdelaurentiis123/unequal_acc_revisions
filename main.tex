...
\begin{figure}
    ...
    \begin{tikzpicture}[scale=2.0]
      ...
      % Centered at origin, semi-major a = 1.4 along x, semi-minor b = 1.0 along y
      % Rotated to put pericenter toward the secondary (right side)
      \draw[thick, red!70!black] (0,0) ellipse (1.4 and 1.0);
      \node[red!70!black, anchor=south] at (0, 1.05) {\footnotesize CBD inner edge (cavity wall)};

      % --- center of mass ---
      \fill[gray] (0,0) circle (0.025);
      \node[gray, anchor=north] at (0, -0.05) {\footnotesize COM};

      % --- binary at apocenter ---
      % For (e_b, q_b) = (0.2, 0.1): primary at d_1 = -0.109a, secondary at d_2 = +1.091a
      % Scale: a_b = 1 in this picture; binary separation 1.2 at apocenter
      \fill[blue] (-0.109, 0) circle (0.06) node[above left, blue] {\scriptsize $M_1$};
      \fill[red]  (+1.091, 0) circle (0.025) node[above, red] {\scriptsize $M_2$};

      % --- r_1 vector: primary to nearest cavity wall (on far side, opposite to secondary) ---
      \draw[<->, blue, thick] (-0.109, 0) -- (-1.4, 0);
      \node[blue, below] at (-0.75, -0.02) {\footnotesize $r_1$};

      % --- r_2 vector: secondary to nearest cavity wall (on the same side) ---
      \draw[<->, red, thick] (+1.091, 0) -- (+1.4, 0);
      \node[red, above] at (+1.25, 0.04) {\footnotesize $r_2$};

      % --- binary axis line ---
      \draw[gray, dashed, thin] (-0.109, 0) -- (+1.091, 0);

      % --- precession indicator ---
      \draw[->, gray!70!black, thick, decoration={markings, mark=at position 0.5 with {\arrow{>}}}, postaction={decorate}]
        (0.0, 1.0) arc[start angle=90, end angle=130, x radius=1.4, y radius=1.0];
      \node[gray!70!black, anchor=south east] at (-0.7, 1.0) {\scriptsize precession};
    \end{tikzpicture}
    \caption{Schematic of the cavity geometry at apocenter for the locked-CBD case (here illustrated for an unequal-mass binary with $q_b \ll 1$). The CBD inner edge (red ellipse) is closer to the secondary on one side. The closest distances from each BH to the cavity wall are $r_1$ (primary, blue) and $r_2$ (secondary, red); in the locked configuration shown, $r_1 > r_2$. As the disk precesses, the major axis of the ellipse rotates, and the orientations of $r_1$ and $r_2$ change accordingly.}
...
Turning to the upper row of \autoref{fig:qdot_heatmap} we find that while most of the $q_b = 1$ simulations display $\langle \dot{q} \rangle$ values consistent with zero\footnote{The slightly negative values are within the standard error of the mean for the $\dot{q}$ time-series ($\sigma \approx 10^{-2}$ for $q_b = 1$, estimated from the variance of $\dot{q}(t)$ divided by the integration duration), and can thus be taken to be zero.}, two simulations stand out: $(e_b, q_b) = (0.2, 1.0)$ and $(0.3, 1.0)$ both show strong positive $\langle \dot{q} \rangle$.
...
\autoref{fig:mdot_edd} and its discussion assumed a binary accretion rate of $1 \dot{M}_{\rm Edd}$. However, such assumption only controls the baseline of the binary accretion rate. While a different assumed binary accretion rate would not change the profiles nor the relative accretion rates of the components, it would affect the absolute amplitude---changing its jet-launching behavior. We display this in \autoref{fig:mini_fig8_jet_regimes}, which shows the jet-regime classification across $(e_b, q_b)$ parameter space at four different binary accretion rates $\dot{M}_b \in \{0.01, 0.1, 1, 10\} \dot{M}_{\rm Edd}$.

\begin{figure*}
    \centering
    \includegraphics[width=\textwidth]{mini_fig8_jet_regimes.pdf}
    \caption{Jet-regime classification across $(e_b, q_b)$ parameter space at four different assumed binary accretion rates: $\dot{M}_b = 0.01, 0.1, 1, 10 \dot{M}_{\rm Edd}$. Cells are color-coded by jet behavior: white = no jet, blue = single jet, purple = dual jet, green = flickering jet. The threshold and duration criteria are the same as in \autoref{fig:mdot_edd}. The qualitative tapestry of regimes shifts dramatically with $\dot{M}_b$: at sub-Eddington rates almost no jets launch, at $1 \dot{M}_{\rm Edd}$ single-jet systems dominate the parameter space, and at super-Eddington rates dual and flickering jets fill the higher-$q_b$ region. The flickering regime --- our novel result --- is broadest at $\dot{M}_b = 10 \dot{M}_{\rm Edd}$ in the high-$q_b$, high-$e_b$ corner.}
    \label{fig:mini_fig8_jet_regimes}
\end{figure*}
...
\begin{figure}
    ...
    \caption{Extended integration of \autoref{eqn:num_integ} from $a_0 = 10^3 R_S$ down to $a = 5 R_S$ for the $(e_b, q_b) = (0.2, 1.0)$ (blue) and $(0.3, 1.0)$ (red) cases, with $M_{\rm tot} = 10^7 M_\odot$ and $\dot{M}_b = 100 \dot{M}_{\rm Edd}$. Top: semi-major axis vs GW frequency. Middle: eccentricity vs frequency, showing initial growth toward the gas-driven equilibrium $e \approx 0.45$ followed by GW circularization. Bottom: mass ratio vs frequency. The mass-ratio deviation from unity established during the gas-dominated phase is preserved through GW-dominated inspiral and persists into the LISA frequency band (gray, $10^{-4}$ to $10^{-1}$ Hz). The minor step in $q(t)$ around $f_{\rm GW} \sim 2 \times 10^{-6}$ Hz reflects the discrete grid resolution of the $\dot{q}_{\rm gas}$ lookup table (rounded to $\Delta q_b = 0.1$); the qualitative result is unchanged.}
    \label{fig:q_evolution_to_LISA}
\end{figure}
